export const featuresData = [
  {
    iconClass: 'flaticon-cv',
    title: 'Αναζήτηση Υπηρεσιών',
    description: `Βρες την υπηρεσία που ικανοποιεί τις ανάγκες σου.`,
  },
  {
    iconClass: 'flaticon-web-design',
    title: 'Επιλογή Επαγγελματία',
    description: `Με τη Doulitsa είναι εύκολο να εντοπίσεις το ιδανικό επαγγελματικό προφίλ.`,
  },
  {
    iconClass: 'flaticon-secure',
    title: 'Τρόποι Πληρωμής',
    description: `Διαλέγεις τον τρόπο πληρωμής που σου δίνει τη μεγαλύτερη ευκολία και ασφάλεια.`,
  },
  {
    iconClass: 'flaticon-customer-service',
    title: 'Επικοινωνία',
    description: `Διαλέγεις με ποιον τρόπο θα επικοινωνήσεις με τον επαγγελματία.`,
  },
];

export const testimonials = [
  {
    id: 'dimitris',
    title: 'Σας συστήνω ανεπιφύλακτα',
    comment:
      '“Η επιχείρηση μου είναι μια μικρή ξενοδοχειακή μονάδα. Η Doulitsa με βοηθάει να βρίσκω επαγγελματίες που δεν θα μπορούσα να προσλάβω σε μόνιμη θέση.”',
    name: 'Δημήτρης Κ.',
    category: 'Ξενοδόχος',
    image:
      'https://res.cloudinary.com/ddejhvzbf/image/upload/v1750071301/Static/dimitris60_sghbb2.webp',
  },
  {
    id: 'katerina',
    title: 'Πολύ καλή δουλειά',
    comment:
      '“Βρήκα γρήγορα έναν επαγγελματία designer για να μου σχεδιάσει ένα καινούργιο logo και έμεινα απόλυτα ικανοποιημένη!”',
    name: 'Κατερίνα Ζ.',
    category: 'Επιχειρηματίας',
    image:
      'https://res.cloudinary.com/ddejhvzbf/image/upload/v1750071212/Static/katerina60_omarhe.webp',
    active: true,
  },
  {
    id: 'marios',
    title: 'Είμαι πολύ ικανοποιημένος',
    comment:
      '“Μέσα από την πλατφόρμα σας μπόρεσα να παρουσιάσω όλη τη δουλειά μου και να με εντοπίσουν νέους πελάτες, αλλά και συνεργάτες.”',
    name: 'Μάριος Θ.',
    category: 'Web Developer',
    image:
      'https://res.cloudinary.com/ddejhvzbf/image/upload/v1750071226/Static/marios60_z8fucv.webp',
  },
];

export const categoriesClassNames = [
  'col-sm-6 col-lg-4 col-xl-3 bdr1 m--1 border-start-0 border-top-0',
  'col-sm-6 col-lg-4 col-xl-3 bdr1 m--1 border-top-0',
  'col-sm-6 col-lg-4 col-xl-3 bdr1 m--1 border-top-0',
  'col-sm-6 col-lg-4 col-xl-3 m--1 border-start-0 border-bottom-0',
  'col-sm-6 col-lg-4 col-xl-3 bdr1 m--1 border-start-0 border-bottom-0',
  'col-sm-6 col-lg-4 col-xl-3 bdr1 m--1 border-bottom-0',
  'col-sm-6 col-lg-4 col-xl-3 bdr1 m--1 border-bottom-0 border-end-0',
  'col-sm-6 col-lg-4 col-xl-3 bdr1 m--1 border-bottom-0 border-end-0',
];

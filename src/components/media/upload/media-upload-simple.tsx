...
const MediaUploadSimple = forwardRef<MediaUploadRef, MediaUploadProps>((props, ref) => {
  const {
    value,
    onChange,
    uploadPreset,
  ...
  const handleFileSelection = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = fileArray.filter(file => {
      ...
      if (!isValidFormat) {
        setUploadError(`Invalid file format: ${extension}. Allowed: ${allowedFormats.join(', ')}`);
        return false;
      }
      
      if (!isValidSize) {
        setUploadError(`File too large: ${Math.round(file.size / 1024 / 1024)}MB. Max: ${Math.round(maxFileSize / 1024 / 1024)}MB`);
        return false;
      }
  ...
  }, [selectedFiles, uploadPreset, folder, type, resources, multiple, onChange]);

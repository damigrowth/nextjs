...
import { AlertCircle, CheckCircle, Loader2, Phone, Globe, MessageCircle, Facebook, Instagram, Linkedin, Youtube, Github, Dribbble } from 'lucide-react';

// Custom X icon
import XCustom from '@/components/icon/x-custom';

// Auth and utilities
import { useAuth, useAuthUser, useAuthLoading } from '@/components/providers/auth';
...
export function PresentationForm() {
  const [state, action, isPending] = useActionState(updateProfilePresentation, initialState);
  ...
  const { formState: { errors, isValid, isDirty }, getValues } = form;
  ...
  useEffect(() => {
    if (state.success) {
      console.log('Presentation form updated successfully - layout will refresh with new data');
    }
  }, [state.success]);
  ...
  return (
    <Form {...form}>
      <form action={handleFormSubmit} className="space-y-6 p-6 border rounded-lg">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <p className="text-sm text-muted-foreground">Προσθέστε έργα από το χαρτοφυλάκιό σας</p>
              <FormControl>
                <MediaUpload
                  ref={mediaRef}
                  value={field.value || []}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex items-center gap-2">
                    <span className="h-4 w-4 text-xs font-bold flex items-center justify-center">Be</span>
                    Behance
                  </FormLabel>
                  <FormControl>
                    <Input

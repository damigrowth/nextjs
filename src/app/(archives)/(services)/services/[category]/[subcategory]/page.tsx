...
export async function generateMetadata({
  ...
  if (!result.success || !result.data.taxonomyData.currentCategory || !result.data.taxonomyData.currentSubcategory) {
    return {
  ...
  const { currentCategory: category, currentSubcategory: subcategory } = result.data.taxonomyData;
...
export default async function SubcategoryPage({
  ...
  return (
    <ArchiveLayout
      ...
      counties={counties}
      basePath={`/services/${categorySlug}/${subcategorySlug}`}
      total={total}
      limit={20}
    >
        ...
        {services.length === 0 ? (
          <div className="text-center py-12">
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              Δεν βρέθηκαν υπηρεσίες στην υποκατηγορία "{taxonomyData.currentSubcategory?.label}"
            </h3>
